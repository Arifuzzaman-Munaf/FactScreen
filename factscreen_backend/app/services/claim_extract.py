...
class ClaimExtractionService:
    ...
    def fetch_google_claims(self, query: str, language_code: str = "en", page_size: int = 10) -> Dict[str, Any]:
        ...
        try:
            ...
            response = requests.get(endpoint, params={**params, "key": self.google_api_key}, timeout=settings.request_timeout)
    ...
    def fetch_rapidapi_claims(self, query: str, limit: int = 10) -> Dict[str, Any]:
        ...
        try:
            ...
            conn.request("GET", f"/{self.fact_checker_endpoint}?query={encoded_query}&limit={limit}", headers=headers)
        ...
        for claim in api_response.get("claims", []):
            for review in claim.get("claimReview", []):
                output.append({
                    ...
                    "other": {k: v for k, v in review.items() if k not in ['textualRating', 'url', 'publisher']}
                })
        return output
        ...
        for claim in api_response.get("data", []):
            for review in claim.get("claim_reviews", []):
                output.append({
                    ...
                    "other": {k: v for k, v in review.items() if k not in ['review_text', 'review_link', 'publisher']}
                })
        return output

    def get_combined_claims(self, query: str, language_code: str = "en", page_size: int = 10) -> List[Dict[str, Any]]:
