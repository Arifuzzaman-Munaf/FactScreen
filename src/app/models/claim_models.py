...
class FilteredClaimResponse(ClaimResponse):
    ...
    query_similarity_score: float = Field(
        ..., description="Similarity score with the query", ge=0.0, le=1.0
    )
...
class FilteredClaimsResponse(BaseModel):
    ...
    claims: List[FilteredClaimResponse] = Field(
        ..., description="List of filtered and classified claims"
    )
...
class FilteredClaimsRequest(ClaimRequest):
    ...
    similarity_threshold: float = Field(
        default=0.75, description="Minimum similarity score", ge=0.0, le=1.0
    )
